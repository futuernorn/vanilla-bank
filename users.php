...
$sid=$user->session_id;
$isCoolDude = false;
if (group_memberships(8,$user->data['user_id'], true) || group_memberships(9,$user->data['user_id'], true)) {
  $isCoolDude = true;
}
...
while($row = $result->fetch_array(MYSQLI_ASSOC)) {
  ...
  if (isset($_GET['noAlts'])) {
    $notePos = strrpos($row['guild_note'], "_alt");
    $officerPos = strrpos($row['officer_note'], "_alt");
    $noteHateSham = strrpos($row['guild_note'], "&#95;alt");
    $officerHateShame = strrpos($row['officer_note'], "&#95;alt");
    if (($notePos !== false) || ($officerPos !== false) || ($noteHateSham !== false) || ($officerHateShame !== false))  // note: three equal signs
      continue;
...
foreach ($common->classes as $class) {
  ...
  $levelSum = 0;
  $averageLevel = 0;
  $query = "SELECT name,level,guild_note,officer_note FROM members WHERE class='$class' AND level>$levelLimit AND last_online<$hoursLimit";
  $result = $common->query($query);
  while($row = $result->fetch_array(MYSQLI_ASSOC)) {
  ...
  $levelSum = 0;
  $averageLevel = 0;
  $query = "SELECT name,level,guild_note,officer_note FROM members WHERE class='$class' AND level>$levelLimit";
  $result = $common->query($query);
  while($row = $result->fetch_array(MYSQLI_ASSOC)) {
    if (in_array($row['name'],$common->bankerList))
      continue;
...
<?php echo $common->makeFabulous("<".GUILDNAME."> - Members");?>
</h1>
...
if (!isset($_GET['noAlts'])) {
?>
<div style='text-align:center;font-size:75%;'><a href='/users.php?noAlts=1'>Ignore Alts</a></div>
<?php
} else {
?>
<div style='text-align:center;font-size:75%;'><a href='/users.php'>Include Alts</a></div>
<?php
}
?>
<br>
...
<div id="graphs" style=' background-color:#000;'>
  ...
  <br />
    ...
    <th>Note</th>		
    <?php echo $isCoolDude ? "<th>Officer Note</th>" : ""; ?>
...
foreach ($members as $member) {
  ...
  echo $isCoolDude ? "<td>".$member['officer_note']."</td>" : "";
